...
Native Android client for the Scheduler native platform split.

## Owns

Kotlin/Compose Scheduler app backed by scheduler-api contracts.

## Native Shell

The first shell lives in `app/src/main/java/com/schedulersystems/scheduler`.
It renders mocked contract-shaped schedule state from `fixtures/mock-schedule.json`
and identifies manager and worker modes.
