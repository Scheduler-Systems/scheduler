...
/**
 * Single-screen phone sign-in: phone-number entry, then SMS-code entry once the code is
 * sent, driven by [AuthUiState.isCodeSent]. One screen / one [AuthViewModel] so the
 * verificationId survives the phone→code transition (the previous two-route split lost it
 * across the per-route `hiltViewModel()` instances, and the code route was never wired).
 */
@Composable
    ...
    Scaffold(snackbarHost = { SnackbarHost(snackbarHostState) }) { padding ->
        Column(
            ...
            if (uiState.isCodeSent) {
                ...
                Button(
                    onClick = { viewModel.onEvent(AuthEvent.VerifyPhoneCode(uiState.smsCode)) },
                    modifier = Modifier
                        .fillMaxWidth()
                        .height(50.dp),
                ...
                Button(
                    onClick = { viewModel.onEvent(AuthEvent.SendPhoneVerification(uiState.phoneNumber)) },
                    modifier = Modifier
                        .fillMaxWidth()
                        .height(50.dp),
